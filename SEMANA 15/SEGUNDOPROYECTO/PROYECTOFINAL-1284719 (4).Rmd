...
```{r}
titanic_data_clean$Titulos <- gsub('(.*, )|([.].*)', '', titanic_data_clean$Titulos) # Get ride of anything that ends in a comma or anything that starts with a period

# se ve cuantas personas hay por cada titulo
titanic_data_clean %>%
    group_by(Titulos, passenger_sex) %>%
    summarize(n = n()) %>%
    arrange(Titulos)
...
```{r}
titanic_data_clean$Titulos[titanic_data_clean$Titulos == 'Mlle']  <- 'Miss'
titanic_data_clean$Titulos[titanic_data_clean$Titulos == 'Ms']    <- 'Miss'
titanic_data_clean$Titulos[titanic_data_clean$Titulos == 'Mme']   <- 'Mrs'

special_people  <- c('Capt', 'Col', 'Dr',  'Lady', 'Major', 'Rev', 'Sir', 'the Countess', 'Don')
titanic_data_clean$Titulos[titanic_data_clean$Titulos %in% special_people]  <- 'Special'
```
...
#creo variables dummy(convertir variables categoricas en variables numericas binarias ) para las variables categoricas
titanic_data_clean <- titanic_data_clean %>%
  mutate(titulo_Mr = as.integer(Titulos == "Mr"),
         titulo_Miss = as.integer(Titulos == "Miss"),
         titulo_Mrs = as.integer(Titulos =="Mrs"),
         titulo_Master = as.integer(Titulos =="Master"),
         titulo_Especial = as.integer(Titulos =="Special")
          )
...
```{r}
titanic_data_clean <- titanic_data_clean %>%
  select(-Age,-Fare,-Embarked,-passenger_class,-passenger_sex,-Titulos,-Female,-AGE,-Fare_i,-AgeCategory)
titanic_data_clean
```
...
```{r}
str(train_data)#con esto muestro la estructura interna del data frame
```

```{r}
#funcion para Normalizar  valores numericos de la columna Fare_log

Normalizar <- function(x) {
 rng <- range(x, na.rm = TRUE)
 (x - rng[1]) / (rng[2] - rng[1])
}
#se Normalizan los datros numericos
train_data$Fare_log <- Normalizar(train_data$Fare_log)
validation_data$Fare_log <- Normalizar(validation_data$Fare_log)

#train_data$PassengerId <- Normalizar(train_data$PassengerId)
#validation_data$PassengerId <- Normalizar(validation_data$PassengerId)
...
#summary(model)
#model <- rpart(passenger_survived ~ ., data = train_data)
summary(model)
```
...
evaluation_data_clean$Titulos <- gsub('(.*, )|([.].*)', '', evaluation_data_clean$Titulos) # Get ride of anything that ends in a comma or anything that starts with a period
...
```{r}
#evaluation_data_clean$Titulos[evaluation_data_clean$Titulos == 'Mlle']  <- 'Miss'
#evaluation_data_clean$Titulos[evaluation_data_clean$Titulos == 'Ms']    <- 'Miss'
#evaluation_data_clean$Titulos[evaluation_data_clean$Titulos == 'Mme']   <- 'Mrs'

special_people  <- c( 'Dr', 'Rev', 'Jonkheer')
evaluation_data_clean$Titulos[evaluation_data_clean$Titulos %in% special_people]  <- 'Special'
```
...
evaluation_data_clean <- evaluation_data_clean %>%
  select(-Fare,-Embarked, -passenger_class,-passenger_sex,-Titulos,-Female,-AGE, -AgeCategory)
evaluation_data_clean
```
...
#se Normalizan los datros numericos
evaluation_data_clean$Fare_log <- Normalizar(evaluation_data_clean$Fare_log)

#evaluation_data_clean$PassengerId <- Normalizar(evaluation_data_clean$PassengerId)
...
#model_test <- rpart(passenger_survived ~ ., data = titanic)
#summary(model)
summary(model_test)
...
predicted_survived <- ifelse(predictions_test > 0.5, "Y", "N")
#predictions_test<- predict(model_test, newdata = evaluation_data_clean, type = 'matrix')
#predicted_survived <- ifelse(predictions_test > 0.5, "Y", "N")
predicted_survived
...
passenger_survived <- as.integer(predicted_survived== "Y") #comparo cada valor de la columna passenger_survived con "Y" TRUE si valor es igual a "Y" y FALSE "N" con as.integer TRUE pasa a 1 y FALSE pasa a 0
passenger_survived
```
...
write.csv(predicciones_data, "predictions9.csv", sep = ",", row.names = FALSE)
