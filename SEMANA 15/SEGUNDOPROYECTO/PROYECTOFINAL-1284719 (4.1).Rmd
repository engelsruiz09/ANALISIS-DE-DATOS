...
```{r}
#calcular el IQR de las edades por titulo
age_stats <- titanic_data_clean %>%
  group_by(Titulos) %>%
  summarize(Q1 = quantile(AGE, 0.25, na.rm = TRUE),
            Q3 = quantile(AGE, 0.75, na.rm = TRUE))

#funcion para imputar un valor aleatorio dentro del IQR por titulo
impute_age_by_title <- function(age, title, age_stats) {
  if (is.na(age)) {
    Q1 <- age_stats$Q1[age_stats$Titulos == title]
    Q3 <- age_stats$Q3[age_stats$Titulos == title]
    random_age <- runif(1, min = Q1, max = Q3)
    return(random_age)
  } else {
    return(age)
  }
...
#aplicar la funcion de imputacion en la columna AGE 
titanic_data_clean$AGE <- mapply(impute_age_by_title, titanic_data_clean$AGE, titanic_data_clean$Titulos, MoreArgs = list(age_stats = age_stats))
titanic_data_clean
```
...
titanic_data_clean <- titanic_data_clean %>%
  select(-passenger_sex, -passenger_class, -Embarked,-Age, -AgeCategory,-AGE,-Fare)
titanic_data_clean
```
...
```{r}
#calcular el IQR de las edades por titulo
age_stats <- evaluation_data_clean %>%
  group_by(Titulos) %>%
  summarize(Q1 = quantile(AGE, 0.25, na.rm = TRUE),
            Q3 = quantile(AGE, 0.75, na.rm = TRUE))
...
#aplicar la funcion de imputacion en la columna AGE 
evaluation_data_clean$AGE <- mapply(impute_age_by_title, evaluation_data_clean$AGE, evaluation_data_clean$Titulos, MoreArgs = list(age_stats = age_stats))
evaluation_data_clean
```
...
write.csv(predicciones_data, "predictions13.csv", sep = ",", row.names = FALSE)
```
