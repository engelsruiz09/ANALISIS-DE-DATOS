...
titanic_data_clean <- titanic_data_clean %>%
  ...
  mutate(#Male = as.integer(passenger_sex == "0"),
         #Female = as.integer(passenger_sex == "1"),
...
```{r}
#convertir embarked en factores
#obtener los niveles únicos de los titulos
...
#model <- glm(passenger_survived ~ passenger_class + passenger_sex + Age + SibSp + Parch + Fare_log + Embarked_S + Embarked_C + Embarked_Q, data = train_data , family = "binomial")

model <- rpart(passenger_survived ~ passenger_class + passenger_sex + Age + SibSp + Parch + Fare_log + Embarked_S + Embarked_C + Embarked_Q, data = train_data)
...
evaluation_data_clean <- evaluation_data_clean %>%
  mutate(#passenger_class_i = as.integer(passenger_class))
         #Male = as.integer(passenger_sex == "M"),
...
model_test <- rpart(passenger_survived ~ passenger_class + passenger_sex + Age + SibSp + Parch + Fare_log + Embarked_S + Embarked_C + Embarked_Q, data = titanic)
...
write.csv(predicciones_data, "predictions18.csv", sep = ",", row.names = FALSE)
